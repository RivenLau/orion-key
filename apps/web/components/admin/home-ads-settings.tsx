...
import { HOME_ADS_CONFIG_KEY, MAX_HOME_ADS, homeAdSettingsEditorSchema, homeAdSettingsSchema, isPosterUrl, isSponsorImage, resolveHomeAds, type HomeAdSettings } from "@/lib/home-sponsors"
...
function readSettings(value?: string): HomeAdSettings | null {
  try {
    return value === undefined ? resolveHomeAds(undefined) : homeAdSettingsEditorSchema.parse(JSON.parse(value))
  } catch { return null }
}
...
export function HomeAdsSettings({ initialValue, onSaved, onReload }: { initialValue?: string; onSaved: (value: string) => void; onReload: () => void }) {
  ...
  const persist = async (next: HomeAdSettings, toggleId?: string) => {
    ...
    if (!parsed.success) {
      ...
      const message = field === "intervalSeconds" ? "ads.invalidInterval"
        : field === "href" ? "ads.invalidLink"
        : field === "name" ? "ads.invalidName"
        : field === "sort_order" ? "ads.invalidOrder"
        : imageFields.some((imageField) => imageField === field) ? "ads.invalidImage"
        : "ads.invalidFields"
      toast.error(t(message))
      return false
    }
    pending.current = true
    if (toggleId) setTogglingId(toggleId)
    else setSaving(true)
    try {
      ...
      if (!toggleId) toast.success(t("ads.saved"))
      return true
    } catch (error) {
      ...
      if (toggleId) setTogglingId(null)
  ...
  const saveForm = async () => {
    if (!form) return
    if (!form.name?.trim()) { toast.error(t("ads.invalidName")); return }
  ...
  const imageInput = (field: ImageField) => (
    <div key={field} className="min-w-0 rounded-lg border border-border p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <label className={buttonClass + " relative cursor-pointer overflow-hidden focus-within:ring-2 focus-within:ring-ring"}>
          {uploading === field ? <Loader2 className="h-4 w-4 animate-spin" /> : <ImagePlus className="h-4 w-4" />}{t("ads.upload")}
          <input type="file" aria-label={`${t("ads.upload")} ${t(`ads.${field}`)}`} accept=".jpg,.jpeg,.png,.webp,.gif" className="absolute inset-0 h-full w-full cursor-pointer opacity-0 disabled:cursor-not-allowed" disabled={busy} onChange={(event) => {
      ...
      {form?.[field] && <>
        <img src={form[field]} alt={t("ads.preview")} referrerPolicy="no-referrer" className="mt-3 h-40 w-full rounded-md bg-muted object-contain" />
  ...
  return <div className="flex flex-col gap-6">
    <fieldset disabled={busy} className="rounded-xl border border-border bg-card p-6 shadow-sm">
      <div className="flex max-w-xl flex-col gap-5">
        ...
        <button type="button" className={primaryClass + " self-start"} aria-disabled={togglingId !== null || undefined} onClick={() => void persist({ ...published, ...options })}><Save className="h-4 w-4" />{t(saving ? "admin.saving" : "admin.saveSettings")}</button>
      </div>
    </fieldset>
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <button type="button" className={primaryClass} disabled={busy || published.items.length >= MAX_HOME_ADS} aria-disabled={togglingId !== null || undefined} onClick={() => { if (!pending.current) setForm({ id: crypto.randomUUID(), name: "", enabled: true, sort_order: Math.min(999999, Math.max(0, ...published.items.map((item) => item.sort_order)) + 1), image: "", href: "" }) }}><Plus className="h-4 w-4" />{t("ads.add")}</button>
    </div>
    <div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          <thead className="border-b border-border bg-muted/50 text-muted-foreground"><tr>{(["ads.preview", "ads.name", "admin.sortOrderLabel", "ads.link", "ads.status", "ads.actions"] as const).map((key) => <th key={key} className="whitespace-nowrap px-4 py-3 font-medium">{t(key)}</th>)}</tr></thead>
          <tbody>{ordered.slice((page - 1) * 10, page * 10).map((item) => <tr key={item.id} className="border-b border-border last:border-0 hover:bg-muted/30">
            ...
            <td className="max-w-52 truncate px-4 py-3"><a href={isPosterUrl(item.href) ? item.href : undefined} target="_blank" rel="noopener noreferrer" title={item.href} className="text-primary hover:underline">{item.href}</a></td>
            <td className="px-4 py-3"><label className="flex items-center gap-2 whitespace-nowrap"><Switch disabled={busy || togglingId === item.id} aria-disabled={busy || togglingId !== null} aria-busy={togglingId === item.id} checked={item.enabled} onCheckedChange={(enabled) => persist({ ...published, items: published.items.map((ad) => ad.id === item.id ? { ...ad, enabled } : ad) }, item.id)} aria-label={t("ads.itemEnabled") + ": " + (item.name || item.id)} /><span className="text-xs text-muted-foreground">{togglingId === item.id ? <Loader2 aria-label={t("admin.saving")} className="h-4 w-4 animate-spin" /> : t(item.enabled ? "ads.on" : "ads.off")}</span></label></td>
            <td className="px-4 py-3"><div className="flex gap-1"><button type="button" disabled={busy} aria-disabled={togglingId !== null || undefined} className="rounded-md p-2 text-muted-foreground hover:bg-accent disabled:opacity-50" aria-label={t("ads.edit") + ": " + (item.name || item.id)} onClick={() => { if (!pending.current) setForm(structuredClone(item)) }}><Edit className="h-4 w-4" /></button><button type="button" disabled={busy} aria-disabled={togglingId !== null || undefined} className="rounded-md p-2 text-destructive hover:bg-destructive/10 disabled:opacity-50" aria-label={t("admin.delete") + ": " + (item.name || item.id)} onClick={() => { if (!pending.current) setDeleteId(item.id) }}><Trash2 className="h-4 w-4" /></button></div></td>
          </tr>)}</tbody>
        </table>
      </div>
    ...
    <Modal open={form !== null} onClose={closeForm} className="max-h-[90vh] max-w-2xl overflow-y-auto overscroll-contain">
      {form && <>
        ...
        <fieldset disabled={busy} className="flex min-w-0 flex-col gap-5 p-6">
          ...
          <div className="grid items-end gap-x-4 gap-y-1.5 sm:grid-cols-2">
            <label className="flex flex-col gap-1.5 text-sm font-medium">{t("admin.sortOrderLabel")}<input type="number" min={0} max={999999} step={1} className={inputClass} value={Number.isNaN(form.sort_order) ? "" : form.sort_order} onChange={(e) => setForm({ ...form, sort_order: e.target.valueAsNumber })} /></label>
            <p className="text-xs text-muted-foreground sm:col-start-1 sm:row-start-2">{t("ads.sortOrderHint")}</p>
            <label className="flex h-10 items-center gap-2 text-sm font-medium sm:col-start-2 sm:row-start-1"><Switch disabled={busy} checked={form.enabled} onCheckedChange={(enabled) => setForm({ ...form, enabled })} />{t("ads.itemEnabled")}</label>
          </div>
          <label className="flex flex-col gap-1.5 text-sm font-medium">{t("ads.link")}<input className={inputClass} type="url" maxLength={2048} placeholder="https://example.com" value={form.href} onChange={(e) => setForm({ ...form, href: e.target.value })} /></label>
          <div className="space-y-3 text-xs leading-relaxed text-muted-foreground">
            {([
              ["ads.specifications", ["ads.formats", "ads.dimensions"]],
              ["ads.versionNotes", ["ads.fallback"]],
            ] as const).map(([heading, entries]) => <div key={heading}>
              <p className="mb-1 font-medium text-foreground">{t(heading)}</p>
              <ul className="list-disc space-y-1 pl-4">{entries.map((key) => <li key={key}>{t(key)}</li>)}</ul>
            </div>)}
          </div>
          {imageInput("image")}
