...
export default function CheckoutPage() {
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      // 仅在微信内置浏览器内时阻止跳转（JSAPI 需微信环境，展示二维码）
      // 普通手机浏览器选微信支付时，网关返回 urlscheme 可直接唤起微信 App
      const isInWechatBrowser = detectPaymentDevice() === "wechat"
      if (isMobileDevice() && payUrlH5 && !selectedPayment.startsWith("usdt_") && !isInWechatBrowser) {
