...
> **API 代理**：`next.config.mjs` 已配置 `rewrites`，前端对 `/api/*` 的请求会自动代理到 `http://localhost:8082`，无需手动处理跨域。如后端端口不同，设置环境变量 `BACKEND_URL=http://localhost:端口号`。

### 6. 验证

- 后端健康检查：`GET http://localhost:8082/api/categories`
...
export SERVER_PORT=8082                        # 默认 8082
```
...
| 环境 | 前端如何到达后端 API |
|------|---------------------|
| **本地开发** | Next.js `rewrites` 代理 `/api/*` → `localhost:8082` |
| **生产部署** | Nginx 反向代理 `/api/*` → `127.0.0.1:8082` |
...
- [ ] Nginx 反向代理已配置（`/api/` → 8082，`/` → 3000，`/uploads/` → 静态目录）
